...
public class Proposer_15 {
    ...
    private final Map<Integer, String> port_acceptor = new HashMap<>();
    private Socket proposer = null;
    private ServerSocket ss = null;
    private boolean needBreak = false;

    Proposer_15(int port, String votingName){
        this.port = port;
    ...
    public void confSettings(){
        ...
        port_acceptor.put(2191,"M11");
        port_acceptor.put(2192,"M12");
        port_acceptor.put(2193,"M13");
        port_acceptor.put(2194,"M14");
        port_acceptor.put(2195,"M15");
        //**********************************************
    }
    ...
    public void commit(String acceptIp, int acceptPort, String votingName) throws IOException,InterruptedException{
        ...
        if(reply.equalsIgnoreCase("Ack")){
            ...
            if(Long.parseLong(pid_cand[0]) > acceptPid){
                ...
                candidateValue = pid_cand[1];
                System.out.println("Proposer " + proposerName + "change the value to :" + candidateValue);
            }
        }
        else{
        ...
        bw.write(acceptPid + "," + candidateValue);
        bw.close();
        fw.close();
    ...
    private void prepareStage(Socket proposer, String message) throws IOException,InterruptedException{
        ...
        if(comingPid > acceptPid){
            if(candidateValue.length() == 0){
                ...
                acceptPid = comingPid;
                bw.write(acceptPid + "," + candidateValue);
                data_out.writeUTF(pid + "," + candidateValue);
            }
            bw.close();
            fw.close();
        }
...
//            if(candidataValue.length()==0){
                ...
                acceptPid = pid;
                candidateValue = canIdPari[0];

                bw.write(pid + "," + candidateValue);
...
//            }
    ...
    public void tellToLearn(String acceptIp, int acceptPort) throws IOException,InterruptedException{
        ...
        DataOutputStream bdos = new DataOutputStream(proposer.getOutputStream());
        bdos.writeUTF(acceptPid + ":" + candidateValue);
    }
